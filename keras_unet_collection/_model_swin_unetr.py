"""
   ...
   This code was modified by Paul Thompson 2024
"""
...
def mlp(x, cf):
    x = keras.layers.Dense(cf["mlp_dim"], activation="gelu")(x)
    x = keras.layers.Dropout(cf["dropout_rate"])(x)
    x = keras.layers.Dense(cf["hidden_dim"])(x)
    x = keras.layers.Dropout(cf["dropout_rate"])(x)
    return x
...
def deconv_block(x, num_filters, strides=2):
    x = keras.layers.Conv2DTranspose(num_filters, kernel_size=2, padding="same", strides=strides)(x)
    return x
...
def build_unetr_2d(cf):
    ...
    inputs = keras.layers.Input(input_shape) ## (None, 256, 3072)

    X = patch_extract((cf["patch_size"],cf["patch_size"]))(inputs)
    ...
    pos_embed = keras.layers.Embedding(input_dim=cf["num_patches"], output_dim=cf["hidden_dim"])(positions) ## (256, 768)
    ...
    ## Reshaping
    z0 = keras.layers.Reshape((cf["image_size"], cf["image_size"], cf["num_channels"]))(inputs)
    ...
    z3 = keras.layers.Reshape(shape)(z3)
    z6 = keras.layers.Reshape(shape)(z6)
    z9 = keras.layers.Reshape(shape)(z9)
    z12 = keras.layers.Reshape(shape)(z12)
    ...
    if upscale < 0: ## Patch size less than 16
        p = 2**abs(upscale)
        z3 = keras.layers.MaxPool2D((p, p))(z3)
        z6 = keras.layers.MaxPool2D((p, p))(z6)
        z9 = keras.layers.MaxPool2D((p, p))(z9)
        z12 = keras.layers.MaxPool2D((p, p))(z12)
